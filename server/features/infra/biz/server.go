...
func (s *ServerService) UpdateServer(g *gin.Context, serverId string, server *schemas.ServerUpdate) (diff map[string]map[string]*ts.OrmDiff, err error) {
	dbServer, err := gen.Server.Where(gen.Server.Id.Eq(serverId), gen.Server.OrganizationId.Eq(global.OrganizationId.Get())).First()
	if err != nil {
		return nil, err
	}
	updateFields := make(map[string]*ts.OrmDiff)
	if server.Name != nil && *server.Name != dbServer.Name {
		updateFields["name"] = &ts.OrmDiff{Before: dbServer.Name, After: *server.Name}
		dbServer.Name = *server.Name
	}
	if server.Status != nil && *server.Status != dbServer.Status {
		updateFields["status"] = &ts.OrmDiff{Before: dbServer.Status, After: *server.Status}
		dbServer.Status = *server.Status
	}
	if server.Manufacturer != nil && *server.Manufacturer != dbServer.Manufacturer {
		updateFields["manufacturer"] = &ts.OrmDiff{Before: dbServer.Manufacturer, After: *server.Manufacturer}
		dbServer.Manufacturer = *server.Manufacturer
	}
	if server.OsVersion != nil && *server.OsVersion != dbServer.OsVersion {
		updateFields["osVersion"] = &ts.OrmDiff{Before: dbServer.OsVersion, After: *server.OsVersion}
		dbServer.OsVersion = *server.OsVersion
	}
	if server.Description != nil && server.Description != dbServer.Description {
		updateFields["description"] = &ts.OrmDiff{Before: dbServer.Description, After: *server.Description}
		dbServer.Description = server.Description
	}
	if server.Cpu != nil && *server.Cpu != dbServer.Cpu {
		updateFields["cpu"] = &ts.OrmDiff{Before: dbServer.Cpu, After: *server.Cpu}
		dbServer.Cpu = *server.Cpu
	}
	if server.Memory != nil && *server.Memory != dbServer.Memory {
		updateFields["memory"] = &ts.OrmDiff{Before: dbServer.Memory, After: *server.Memory}
		dbServer.Memory = *server.Memory
	}
	if server.Disk != nil && *server.Disk != dbServer.Disk {
		updateFields["disk"] = &ts.OrmDiff{Before: dbServer.Disk, After: *server.Disk}
		dbServer.Disk = *server.Disk
	}
	if helpers.HasParams(g, "rackId") && server.RackId != dbServer.RackId {
		updateFields["rackId"] = &ts.OrmDiff{Before: dbServer.RackId, After: *server.RackId}
	...
	if helpers.HasParams(g, "rackPosition") {
		...
		if position != *dbServer.RackPosition {
			updateFields["rackPosition"] = &ts.OrmDiff{Before: dbServer.RackPosition, After: position}
...
func (s *ServerService) GetAllServerIdsBySiteId(siteId string) ([]string, error) {
	var result []string
	err := gen.Server.Select(gen.Server.Id).Where(gen.Server.SiteId.Eq(siteId), gen.Server.OrganizationId.Eq(global.OrganizationId.Get())).Scan(&result)
	return result, err
}
...
func (s *ServerService) GetById(serverId string) (*schemas.Server, error) {
	...
	if err != nil {
		core.Logger.Error("[infraDeviceService]: failed to get server op status", zap.Error(err))
	}
